    ...
    const fetchCategoryProducts = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/public/category/${slug}`,
          { cache: "no-store" }
        );
...
const handleApplyFilter = () => {
  ...
  setIsFilterApplied(true);   // ✅ mark as active
  setFilterOpen(false);
};
        ...
        <div className="bg-white min-h-screen flex flex-col">
            ...
            <main className="flex-grow w-full px-4 md:px-8 py-10">
                ...
                <div className="flex items-center justify-center space-x-2 text-base sm:text-lg text-gray-500 font-medium mb-6">
                    ...
                    <span className="text-black hover:underline cursor-pointer">Collection</span>
                    <span className="text-gray-400">&gt;</span>
...
<label className="flex items-center gap-2">
  ...
  max={maxPrice}
  onChange={handleMinChange}
  onBlur={(e) => setMinPrice(Number(e.target.value) || 0)}
...
/>
  ...
  max="10000"
  onChange={handleMaxChange}
  onBlur={(e) => setMaxPrice(Number(e.target.value) || 0)}
  className="w-20 outline-none"
/>
                    ...
                    {/* Clear & Apply */}
                    <div className="pt-4">
                        <button onClick={handleClearFilter} className="text-sm font-semibold underline">CLEAR ALL</button>
